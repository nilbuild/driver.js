...
type Config = {
  ...
  // Possible options are 'close', 'nextStep', and 'preventDismiss'. (default: 'close')
  // - 'close': Closes the driver when overlay is clicked
  // - 'nextStep': Moves to the next step when overlay is clicked
  // - 'preventDismiss': Prevents any action when overlay is clicked
